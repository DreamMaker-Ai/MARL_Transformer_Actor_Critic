...
from worker_hierarchy import Worker
from replay_hierarchy import Replay
from learner_hierarchy import Learner
from tester_hierarchy import Tester
...
def write_config(config, num_workers):
    ...
    config_list = {
        ...
        'commander_grid_size': config.commander_grid_size,
        'commander_update_cycle': config.command_update_cycle,
        'commander_oservation_channels': config.commander_observation_channels,
...
def main(is_debug, num_workers=8):
    ...
    pool_of_networks = config.pool_of_networks

    # workerのインスタンスをnum_workers個生成
    workers = [Worker.remote(pid=i, pool_of_networks=pool_of_networks)
    ...
    wip_workers = [worker.rollout_and_collect_trajectory.remote(current_weights, pool_of_networks)
    ...
    for _ in range(config.worker_rollouts_before_train):
        ...
        wip_workers.extend([workers[pid].rollout_and_collect_trajectory.remote(
            current_weights, pool_of_networks)])
        # 新しいobject refsを追加
    ...
    wip_learner = learner.update_network.remote(
        minibatchs=minibatchs, pool_of_networks=pool_of_networks)  # network更新  ObjectRef
    ...
    while update_cycles <= config.num_update_cycles:
        ...
        if finished_worker:
            ...
            wip_workers.extend(
                [workers[pid].rollout_and_collect_trajectory.remote(
                    current_weights, pool_of_networks)])
        ...
        if finished_learner:
            ...
            current_weights, p_loss, q_loss, alpha_loss, pool_of_networks = \
            ...
            wip_learner = learner.update_network.remote(
                minibatchs=minibatchs, pool_of_networks=pool_of_networks)
