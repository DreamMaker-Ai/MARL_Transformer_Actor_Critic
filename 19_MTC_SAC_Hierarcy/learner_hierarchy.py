...
class Learner:
    def __init__(self):
        ...
        self.mtc = MarlTransformerHierarchyModel(config=self.env.config)
    ...
    def define_network(self):
        ...
        if self.env.config.continual_learning:
    ...
    def update_network(self, minibatchs, pool_of_networks):
        """
        ...
        if self.count % 5000 == 0:  # Default=5000
            ...
            # add network id to pool_of_networks
            pool_of_networks.append(self.count)

            with open('pool_of_networks.pickle', mode='wb') as fo:
                pickle.dump(pool_of_networks, fo)
