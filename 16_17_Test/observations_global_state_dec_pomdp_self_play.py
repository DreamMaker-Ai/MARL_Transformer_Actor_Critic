...
def get_global_observation(env):  # For reds global_state
...
def get_global_blues_observation(env):  # For blues global_state
    red_normalized_force, red_efficiency = compute_global_red_maps(env)  # (g,g)
    blue_normalized_force, blue_efficiency = compute_global_blue_maps(env)  # (g,g)
...
def compute_red_pos_maps(env):
    """
    map of agent position in agent set (Not used?)
    """
    try:
...
def get_observations(env):
    """ Observation of POMDP red-agents """
...
def get_blue_observations_po_0(env):
    """
    Observation of POMDP blue-agents
    observations: {agent-id: (env.config.fov * 2 + 1,
                              env.config.fov * 2 + 1,
                              env.config.observation_channels)}
     normalized by arc-tan().  4CH
    ...
    for blue in env.blues:
        if blue.alive:
            ...
            observations[blue.id] = np.concatenate(
                [
                    blue_normalized_force,
                    blue_efficiency,
                    red_normalized_force,
