...
class Learner:
    ...
    def define_network(self):
        ...
        global_ch = self.env.config.global_observation_channels
        ...
        global_obs = np.random.rand(global_obs_shape[0],
                                    global_obs_shape[1],
                                    global_obs_shape[2])

        global_state = np.expand_dims(global_obs, axis=0).astype(np.float32)
        # (1,g,g,global_ch*global_n_frames)

        mask = make_mask(alive_agents_ids=alive_agents_ids,
                         max_num_agents=max_num_agents)  # (1,n)

        self.mtc([padded_obs, global_state], mask, training=False)
        self.target_mtc([padded_obs, global_state], mask, training=False)
    ...
    def update_network(self, minibatchs):
        ...
        for minibatch in minibatchs:
            ...
            [next_action_logits, [next_q1, next_q2]], _ = \
                self.target_mtc([next_states, next_global_state], next_masks, training=False)
            ...
            targets = team_reward + (1. - team_done) * self.gamma * v  # (b,1)
            ...
            with tf.GradientTape() as tape:
                ...
                [action_logits, [q1, q2]], _ = \
                    self.mtc([states, global_state], masks, training=False)
                # (b,n,action_dim) [(b,n,action_dim), (b,n,action_dim)]
        ...
        # Save model
        if self.count % 10000 == 0:  # Default=5000
