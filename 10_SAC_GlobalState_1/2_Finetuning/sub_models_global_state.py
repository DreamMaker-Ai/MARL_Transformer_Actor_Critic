...
class SoftQModelGlobalState(tf.keras.models.Model):
    ...
    ==================================================================================================
     ...
     tf.expand_dims (TFOpLambda)    (None, 1, 256)       0           ['input_2[0][0]']

     input_1 (InputLayer)           [(None, 15, 256)]    0           []

     tf.tile (TFOpLambda)           (None, 15, 256)      0           ['tf.expand_dims[0][0]']

     tf.concat (TFOpLambda)         (None, 15, 512)      0           ['input_1[0][0]',
                                                                      'tf.tile[0][0]']

     time_distributed_10 (TimeDistr  (None, 15, 768)     393984      ['tf.concat[0][0]']
     ibuted)

     time_distributed_13 (TimeDistr  (None, 15, 768)     393984      ['tf.concat[0][0]']
     ibuted)

     time_distributed_11 (TimeDistr  (None, 15, 256)     196864      ['time_distributed_10[0][0]']
     ibuted)

     time_distributed_14 (TimeDistr  (None, 15, 256)     196864      ['time_distributed_13[0][0]']
     ibuted)

     time_distributed_12 (TimeDistr  (None, 15, 5)       1285        ['time_distributed_11[0][0]']
     ibuted)

     time_distributed_15 (TimeDistr  (None, 15, 5)       1285        ['time_distributed_14[0][0]']
     ibuted)

     tf.math.multiply (TFOpLambda)  (None, 15, 5)        0           ['time_distributed_12[0][0]']

     tf.math.multiply_1 (TFOpLambda  (None, 15, 5)       0           ['time_distributed_15[0][0]']
     )
    ...
    def call(self, inputs, mask, training=True):
        ...
        """ Q2 """
        x10 = self.dense10(x)  # (None,n,3*hidden_dim)=(None,n,768)
