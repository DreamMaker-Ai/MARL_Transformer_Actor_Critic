...
def learn(num_workers=8, is_debug=False):
    ...
    [_, _], scores = global_policy(padded_obs, mask, training=False)  # build
    ...
    """ get the weights of global policy """
    ...
    while update_cycles <= env.config.num_update_cycles:
        ...
        for i in range(num_workers):
            ...
            advantages.append(trajectories[i]["advantage"])  # append (b,1)
            expected_returns.append(trajectories[i]["R"])  # append (b,1)
        ...
        advantages = np.array(advantages, dtype=np.float32)  # (w,b,1)
        expected_returns = np.array(expected_returns, dtype=np.float32)  # (w,b,1)
        ...
        advantages = advantages.reshape([data_size, 1])
        expected_returns = expected_returns.reshape([data_size, 1])

        """ 
        3. Use mini_batch (update_batch_size=ub) to compute loss and grads, 
        ...
        for i in range(opt_iter):
            ...
            [old_policy_probs, old_value], _ = \
                old_global_policy(states[idx], masks[idx], training=False)
            # (ub,n,action_dim), (ub,1)

            with tf.GradientTape() as tape:
                [policy_probs, value], _ = global_policy(states[idx], masks[idx], training=False)
                # (ub,n,action_dim), (ub,1)
                ...
                """ Compute Value loss """
                vpred_clipped = old_value + \
                                tf.clip_by_value(value - old_value,
                                                 -env.config.clip_range, env.config.clip_range)
                # (ub,1)

                vloss = tf.maximum(tf.square(value - expected_returns[idx]),
                                   tf.square(vpred_clipped - expected_returns[idx]))  # (ub,1)

                vloss = tf.squeeze(vloss, axis=-1)  # (ub,)
