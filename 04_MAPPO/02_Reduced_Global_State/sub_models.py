...
class ValueModel(tf.keras.models.Model):
    ...
    :param hidden_dim=256
    :param max_num_agents=15=n
    :return: Global_Value, (None,1)

    Model: "global_value_model"
    _________________________________________________________________
     Layer (type)                Output Shape              Param #
    =================================================================
     input_1 (InputLayer)        [(None, 15, 256)]         0

     conv1d (Conv1D)             (None, 15, 32)            8224
    ...
    def __init__(self, config, **kwargs):
        ...
        self.config = config
        self.num_agents = self.config.max_num_red_agents
        self.hidden_dim = self.config.hidden_dim

        self.conv1d = tf.keras.layers.Conv1D(filters=32,
                                             kernel_size=1,
                                             strides=1,
                                             activation='relu',
                                             padding='same',
        ...
        self.reshape = tf.keras.layers.Reshape(target_shape=(self.num_agents * 32,))
    ...
    def call(self, inputs, training=True):
        x1 = self.conv1d(inputs)  # (None, n, 32)
        x1 = self.reshape(x1)  # (None,n*32)
        x1 = self.dense2(x1)  # (None,hidden_dim)
        global_value = self.dense3(x1)  # (None,1)
        return global_value
...
def main():
    ...
    values = value_model(features_mha)

    """ remove tf.function for summary """
    """
    value_model.build_graph().summary()

    tf.keras.utils.plot_model(
        value_model.build_graph(),
        to_file=dir_name + '/global_value_model',
        show_shapes=True,
        show_layer_activations=True,
        show_dtype=True,
        dpi=96 * 3
