...
class Worker:
    ...
    def rollout_and_collect_trajectory(self, weights):
        ...
        1. Rolloutして、batch_size分のデータを収集
            ...
            trajectory["global_r"]: (b,1)
            trajectory["global_done"]: (b,1), bool
        ...
        2. Compute advantage & expected_return
            trajectory['vpred']: values, (b,1)
            trajectory['vpred2']: next_values, (b,1)
            trajectory['advantage']: (b,1)
            trajectory["R"]: expected_return, (b,1)
        """
    ...
    def _rollout(self):
        ...
        for i in range(self.batch_size):
            ...
            trajectory["global_r"].append(reward)  # append float
            trajectory["global_done"].append(done)  # append bool
        ...
        # (b,n), bool
        trajectory["global_r"] = np.vstack(trajectory["global_r"]).astype(np.float32)
        # (b,1)
        trajectory["global_done"] = np.vstack(trajectory["global_done"]).astype(bool)
        # (b,1), bool
    ...
    def _compute_advantage(self, trajectory):
        ...
        [_, value], _ = \
            self.policy(trajectory["s"], trajectory["mask"], training=False)  # value:(b,1)
        trajectory["vpred"] = value.numpy()  # (b,1)

        [_, value2], _ = \
            self.policy(trajectory["s2"], trajectory["mask2"], training=False)  # value2:(b,1)
        trajectory["vpred2"] = value2.numpy()  # (b,1)

        is_nonterminal = 1 - trajectory["global_done"].astype(np.float32)  # (b,1)

        # deltas = r + gamma*(1-dones)V(s') - V(s)
        deltas = trajectory["global_r"] + \
                 self.gamma * is_nonterminal * trajectory['vpred2'] - trajectory['vpred']  # (b,1)

        # compute advantages(GAE) & expected_returns
        advantages = np.zeros_like(deltas, dtype=np.float32)  # (b,1)

        last_gae = 0
        for idx in reversed(range(self.batch_size)):
            last_gae = deltas[idx, 0] + \
                       self.gamma * self.gae_lambda * is_nonterminal[idx, 0] * last_gae
            # scaler
            advantages[idx] = last_gae  # scaler

        # GAE
        trajectory["advantage"] = advantages  # (b,1)

        # Expected_return
        trajectory["R"] = trajectory["advantage"] + trajectory["vpred"]  # (b,1)
