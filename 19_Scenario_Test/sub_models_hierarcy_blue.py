...
class CNNModel(tf.keras.models.Model):
    ...
    def build_graph(self, mask):
        ...
        x0 = tf.keras.layers.Input(
            shape=(self.config.max_num_blue_agents,
                   2 * self.config.fov + 1,
                   2 * self.config.fov + 1,
        ...
        x1 = tf.keras.layers.Input(
            shape=(self.config.max_num_blue_agents, 2 * self.config.n_frames)
        )
...
class PolicyModel(tf.keras.models.Model):
    ...
    _________________________________________________________________
     Layer (type)                Output Shape              Param #
    =================================================================
     time_distributed_7 (TimeDis  multiple                 61632
     tributed)
    ...
    def call(self, inputs, mask, training=True):
        # inputs: [features of agents, commander feature]
        #   =[(None,n,hidden_dim),(None,4*hidden_dim)]=[(None,15,64),(None,256)]
        ...
        """ Concatenate agents_feature and command_feature """
        x = inputs[0]  # (None,n,hidden_dim)=(1,15,64)
        x_command = inputs[1]  # (None, 4*hidden_dim)=(1,256)

        x_command = tf.expand_dims(x_command, axis=1)  # (None,1,4*hidden_dim)=(1,1,256)

        mult = tf.constant([1, self.config.max_num_blue_agents, 1])  # (3,)
        x_command = tf.tile(x_command, mult)  # (None,n,4*hidden_dim)=(1,15,256)

        x = tf.concat([x, x_command], axis=-1)  # (None,n,5*hidden_dim)=(None,n,320)

        """ Logits """
        x1 = self.dense1(x)  # (None,n,3*hidden_dim)=(None,n,192)
        # x1 = self.dropoout1(x1, training=training)
        x1 = self.dense2(x1)  # (None,n,64)
...
def main():
    from battlefield_strategy_hierarchy_scenario_test import BattleFieldStrategy  # used in main
    ...
    global_n_frames = config.global_n_frames  # 4

    global_state_shape = (grid_size, grid_size, global_ch * global_n_frames)  # (15,15,24)

    global_state = np.ones(shape=global_state_shape)  # (15,15,24)
    global_state = np.expand_dims(global_state, axis=0)  # (1,15,15,24)

    global_cnn = GlobalCNNModel(config=config)
    global_feature = global_cnn(global_state)  # (1,4*hidden_dim)=(1,256)
    ...
    max_num_agents = config.max_num_blue_agents  # 15
    ...
    policy_logits = policy_model([features_mha, command_feature], mask)
