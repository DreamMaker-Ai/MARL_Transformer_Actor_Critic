...
class Tester:
    def __init__(self):
        ...
        # Make an actor-critic network (MTD)
    ...
    def reset_states(self, observations, global_state):
        ...
        # For agents in Env
             ...
             -> observations[red.id]: [obs, pos]
                    :obs: (2*fov+1,2*fov+1,channels)
                    :pos: (2*n_frames,)

             -> generate deque of length=n_frames
             self.frames[red.id]: deque[(2*fov+1,2*fov+1,channels),...], len=n_frames

             -> transform to observatios
             self.padded_obs: [(1,2*fov+1,2*fov+1,channels*n_frames),(1,2*n_frames)],...], len=n
        ...
        for idx in range(self.env.config.max_num_red_agents):
            ...
            if idx in self.alive_agents_ids:
                ...
                # [(2*fov+1,2*fov+1,ch),...], len=n_frames

                obs = np.concatenate(self.frames[agent_id], axis=2).astype(np.float32)
                # (2*fov+1,2*fov+1,ch*n_frames)

                red = self.env.reds[idx]
                self.pos_frames[agent_id] = deque([(red.pos[0] / self.env.config.grid_size,
                                                    red.pos[1] / self.env.config.grid_size)]
                                                  * self.n_frames,
                                                  maxlen=self.n_frames)
                pos = np.concatenate(
                    self.pos_frames[agent_id], axis=0).astype(np.float32)  # (2*n_frames,)

            else:
                obs = np.zeros(self.obs_shape)  # (2*fov+1,2*fov+1,ch*n_frames)
                pos = np.zeros((2 * self.n_frames,)).astype(np.float32)

            obs = np.expand_dims(obs, axis=0)  # add batch_dim, (1,2*fov+1,2*fov+1,ch*n_frames)
            pos = np.expand_dims(pos, axis=0)  # (1,2*n_frames)

            self.padded_obss.append([obs, pos])
        ...
        # add batch_dim, (1,g,g,global_ch*global_n_frames)
        ...
        # Build policy
        self.policy([self.padded_obss, self.global_state], self.masks, training=False)
    ...
    def test_play(self, current_weights):
        ...
        for i_episode in range(self.env.config.max_episodes_test_play):
            ...
            while not dones['all_dones']:
                ...
                acts, scores = \
                    self.policy.sample_actions(self.padded_obss, self.masks, training=False)
                ...
                for idx in range(self.env.config.max_num_red_agents):
                    ...
                    if idx in next_alive_agents_ids:
                        ...
                        next_obs = np.zeros(self.obs_shape)  # (2*fov+1,2*fov+1,ch*n_frames)
                        next_pos = np.zeros((2*self.n_frames,))  # (2*n_frames,)

                    next_obs = np.expand_dims(next_obs, axis=0)  # (1,2*fov+1,2*fov+1,ch*n_frames)
                    next_pos = np.expand_dims(next_pos, axis=0)  # (1,2*n_frames)

                    next_padded_obss.append([next_obs, next_pos])
                    # [[(1,2*fov+1,2*fov+1,ch*n_frames).(1,2*n_frames)],...], len=n
...
def main(is_debug):
    ...
    is_debug = is_debug  # True for debug
    ...
    for i in range(max_num_agents):
        if i in alive_agents_ids:
            obs = np.random.random(obs_shape).astype(np.float32)  # (2*fov+1,2*fov+1,ch*n_frames)
    ...
    dummy_policy([agents_obs, global_state], masks, training=False)  # build
