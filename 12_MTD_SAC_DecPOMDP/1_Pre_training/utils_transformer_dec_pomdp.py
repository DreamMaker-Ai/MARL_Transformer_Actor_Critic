...
"""
    ...
    Copied "make_po_id_mask" from "POMDP-Decentralized"
"""
...
def experiences2per_agent_list_dec_pomdp(self, experiences):
    """
    experiences には、batch (=16) 分の transition が入っている。

    :input:
        experience.obss = [[obs of agent_1: (1,15,15,6), pos of agent_1: (1,8)],
                           [obs of agent_2: (1,15,15,6), pos of agent_2: (1,8)],
                              ... ], len=n
    ...
    for experience in experiences:
        for i in range(self.env.config.max_num_red_agents):
            # append agent_i obs: (1,2*fov+1,2*fov+1,ch*n_frames)
            agent_obs[i].append(experience.obss[i][0])

            # append agent_i pos: (1,2*n_frames)]
            agent_pos[i].append(experience.obss[i][1])
            ...
            # append agent_i next_state: (1,2*fov+1,2*fov+1,ch*n_frames)
            agent_next_obs[i].append(experience.next_obss[i][0])

            # append agent_i next_pos: (1,2*n_frames)]
            agent_next_pos[i].append(experience.next_obss[i][1])
    ...
    return agent_obs, agent_pos, agent_action, agent_reward, agent_next_obs, agent_next_pos, \
           agent_done, team_r, team_done, agent_mask, agent_next_mask, global_state, \
           next_global_state


def per_agent_list2input_list_dec_pomdp(self, agent_obs, agent_pos, agent_action, agent_reward,
                                        agent_next_obs, agent_next_pos, agent_done, team_r,
                                        team_done, agent_mask, agent_next_mask,
                                        global_state, next_global_state):
    """
    per_agent_list -> input list to policy network
    :return:
        obss: [[(b,2*fov+1,2*fov+1,ch*n_frames),(b,2*n_frames)], ...], len=n
        actions: [(b,), ...], len=n
        rewards: [(b,), ...], len=n
        next_obss: [[(b,2*fov+1,2*fov+1,ch*n_frames),(b,2*n_frames)], ...], len=n
        dones: [(b,), ...], len=n, bool
        team_rs: (b,1)
        team_dones: (b,1), bool
    ...
    for i in range(self.env.config.max_num_red_agents):
        obss.append([
            np.concatenate(agent_obs[i], axis=0),
            np.concatenate(agent_pos[i], axis=0)
        ])  # append [(b,5,5,16),(b,8)]
        ...
        next_obss.append([
            ...
            np.concatenate(agent_next_pos[i], axis=0)
        ])  # append [(b,5,5,16),(b,8)]
