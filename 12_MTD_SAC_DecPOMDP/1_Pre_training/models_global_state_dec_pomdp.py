...
from sub_models_global_state_dec_pomdp import CNNModel, MultiHeadAttentionModel, PolicyModel, \
...
class MarlTransformerGlobalStateModel(tf.keras.models.Model):
    ...
    :inputs: [agents_obs, global_obs], masks
                :agents_obs: [[obs,pos],...], len=n
                    :obs:(None,2*fov+1,2*fov+1,ch*n_frames)=(None,5,5,16)
                    :pos:(None,2*n_frames)=(None,8)
                :global_obs: (None,g,g,global_ch*global_n_frames)=(None,15,15,6)
    ...
    def mha_block(self, mha, features, masks, training=False):
        ...
        for i in range(self.config.max_num_red_agents):
            query_feature = features[i]  # (None,hidden_dim)=(1,64)
    ...
    def value_block(self, features_mha, global_feature, masks, training=False):
        """
        :param features_mha:  [(None,hidden_dim),...], len=n
        :param global_feature:  (None,hidden_dim*4)
        :param masks: [(None,1,n),...], len=n
    ...
    def call(self, x, masks, training=False):
        ...
        """
        agents_obs = x[0]  # [[(None,5,5,16),(None,8)],...], len=n
        global_state = x[1]  # (None,15,15,6)

        """ Global feature by Global CNN layer """
        global_feature = self.global_cnn(global_state)  # (None,hidden_dim*4)=(1,256)
    ...
    def sample_actions(self, x, masks, training=False):
        ...
        scores = [scores1, scores2]  # [[(None,num_heads,n),...], [(None,num_heads,n),...]]
    ...
    def build_graph(self, masks):
        x = [
            [tf.keras.layers.Input(
                ...
                tf.keras.layers.Input(shape=(2 * self.config.n_frames,))]
