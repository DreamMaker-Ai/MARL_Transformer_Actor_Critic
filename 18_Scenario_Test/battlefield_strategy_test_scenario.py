...
from config_selfplay_2_scenario import Config
from generate_agents_for_test_scenario import generate_red_team, generate_blue_team
...
from models_global_state_mtc_dec_pomdp_blue import MarlTransformerGlobalStateModel
...
class BattleFieldStrategy(gym.Env):
    def __init__(self):
        ...
        self.blue_network_id = self.config.blue_id
    ...
    def build_blue_policy(self):  # copied from tester.main(), then modified.
        ...
        # Load model
        load_dir = Path(__file__).parent / 'blues_test_models'
        load_name = '/model_' + str(self.config.pool_of_test_blue_networks[-1]) + '/'
        dummy_policy.load_weights(str(load_dir) + load_name)

        load_name = '/alpha_' + str(self.config.pool_of_test_blue_networks[-1]) + '.npy'
        logalpha = np.load(str(load_dir) + load_name)
        logalpha = tf.Variable(logalpha)

        return dummy_policy, logalpha

    def reset(self, pool_of_networks):
        ...
        if len(pool_of_networks) > 1:  # worker
            ...
            if self.blue_network_id != 0:  # chekpoints
                ...
                load_name = '/alpha_' + str(self.blue_network_id) + '.npy'
    ...
    def initialize_blue_obss_and_poss(self, blue_observations, global_observation):
        ...
        self.blue_padded_obss = \
            make_blues_padded_obs(max_num_agents=self.config.max_num_blue_agents,
                                  obs_shape=self.obs_shape,
                                  raw_obs=obss)  # (1,n,2*fov+1,2*fov+1,ch*n_frames)=(1,15,5,5,16)

        self.blue_padded_poss = \
            make_blues_padded_pos(max_num_agents=self.config.max_num_blue_agents,
    ...
    def step(self, actions):
        ...
        """ When blue stationary """
        if self.blue_network_id == 0:  # stationary blues -> action=0
            for key in blue_actions.keys():
                blue_actions[key] = 0
