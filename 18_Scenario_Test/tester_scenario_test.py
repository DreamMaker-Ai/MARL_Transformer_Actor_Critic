...
from battlefield_strategy_test_scenario import BattleFieldStrategy
...
class Tester:
    def __init__(self):
        ...
        self.pool_of_networks = self.env.config.pool_of_test_blue_networks

        ### Initialize above Nones
        observations, global_state = self.env.reset(pool_of_networks=self.pool_of_networks)
        ...
        for i_episode in range(self.env.config.max_episodes_test_play):
            ...
            while not dones['all_dones']:
                ...
                if dones['all_dones']:
                    ...
                    observations, global_state = self.env.reset(self.pool_of_networks)
                    self.reset_states(observations, global_state)
...
def main(is_debug):
    ...
    pool_of_networks = env.config.pool_of_test_blue_networks
