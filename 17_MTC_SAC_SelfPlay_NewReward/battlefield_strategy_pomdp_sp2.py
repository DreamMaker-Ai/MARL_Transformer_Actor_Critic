...
from rewards_sp2 import get_consolidation_of_force_rewards, get_economy_of_force_rewards
...
from engage_sp2 import engage_and_get_rewards, compute_engage_mask, get_dones
...
class BattleFieldStrategy(gym.Env):
    ...
    def step(self, actions):
        ...
        if len(x1) > 0:
            ...
            rewards, infos, eliminate_blue_rewards = \
                engage_and_get_rewards(self, x1, y1, rewards, infos)

            for key in eliminate_blue_rewards.keys():
                rewards[key] += self.symlog(eliminate_blue_rewards[key])
        ...
        if done:
            ...
            rewards = self.done_rewards(infos, rewards)
    ...
    def done_rewards(self, infos, rewards):
        if infos["win"] == "reds":
            for red in self.reds:
                if red.alive:
                    rewards[red.id] += self.symlog(50.0)
