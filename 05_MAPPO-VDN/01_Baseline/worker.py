...
class Worker:
    ...
    def _compute_advantage(self, trajectory):
        ...
        trajectory["vpred"] = tf.reduce_sum(values, axis=1)  # (b,1)

        [_, values2], _ = \
            self.policy(trajectory["s2"], trajectory["mask2"], training=False)  # values2:(b,n,1)
        trajectory["vpred2"] = tf.reduce_sum(values2, axis=1)  # (b,1)
